...
void print_symbol(Elf64_Sym *symbol, char *section_str, char *symbol_str) {
  ...
  printf("%016lx\t%lu\t%s\t%s\t%s\t%s\t%s\n", symbol->st_value, symbol->st_size,
         type_str, bind_str, vis_str, section_str, symbol_str);
}
...
int main(int argc, char *argv[]) {
  ...
  Elf64_Ehdr *elf_header = mmap(NULL, s.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
  ...
  for (int i = 0; i < elf_header->e_shnum; i++) {
    if (section_headers[i].sh_type == SHT_SYMTAB) {
      Elf64_Shdr strtab_header = section_headers[section_headers[i].sh_link];
      char *strtab = (char *)elf_header + strtab_header.sh_offset;

      Elf64_Sym *symbols =
          (Elf64_Sym *)((char *)elf_header + section_headers[i].sh_offset);
      ...
      for (int j = 0; j < count; j++) {
        ...
        print_symbol(
            ...
            symbols[j].st_shndx != 0
                ? &shstrtab[section_headers[symbols[j].st_shndx].sh_name]
                : "UND",
